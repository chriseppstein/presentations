$(function() {
  ...
  $('select.format').html("<option selected>expanded</option><option>compact</option><option>compressed</option><option>nested</option><option>sourcemap</option>");
  ...
  $('.live-code').each(function(i, el) {
    ...
    $(el).find("select.format").after("<label class='sourcemap'><input type='checkbox' name='sourcemap' value='sourcemap'>Sourcemap</label>");
    ...
    var updater = function() {
      var sourceEl = $(el).find('.sass .source').first()
      var source = sourceEl.val();
      var id = sourceEl.attr("id");
      var sourcemap = $(el).find("label.sourcemap input:checked").val();
      var request = $.ajax({
        url: "/compile",
        type: "POST",
        data: {sass: source, style: $(el).find("select.format").val(), name: id, sourcemap: sourcemap},
        dataType: "text"
      });
      request.done(function(result) {
        ...
        if (sourcemap) {
          var style = "<link rel='stylesheet' href='/output/"+id+".css'>";
        } else {
          var style = "<style>"+result+"</style>";
        }
        displayEl.html(style+$(htmlEl()).val());
      });
      // It happens, ok?
    ...
    $(el).find("label.sourcemap input").change(updater);
    updater();
  });
});
